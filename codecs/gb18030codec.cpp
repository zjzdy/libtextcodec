...
#define ZValidChar(u)        ((u) ? (u) : static_cast<ushort>(SpecialCharacter::ReplacementCharacter))
...
#define IsUDA3(a, b)        (InRange((a), 0xA1, 0xA7) && InRange((b), 0x40, 0xA0) && ((b) != 0x7F))
	...
	static uint Gb18030ToUnicode(const uchar *gbstr, int& len);
	static int UnicodeToGb18030(uint unicode, uchar *gbchar);
	int UnicodeToGbk(uint unicode, uchar *gbchar);

	Gb18030Codec::Gb18030Codec()
	{
	}

	string Gb18030Codec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
	{
		char replacement = '?';
		int high = -1;
		if (state) {
		...
		for (int i = 0; i < len; i++) {
			...
			uchar buf[4];
			if (high >= 0) {
				if (UCS4Tool::isLowSurrogate(uc[i])) {
					// valid surrogate pair
					++i;
					uint u = UCS4Tool::surrogateToUcs4(high, uc[i]);
			...
			if (IsLatin(ch)) {
				// ASCII
				*cursor++ = ch;
			} else if (UCS4Tool::isHighSurrogate(uc[i])) {
	...
	u16string Gb18030Codec::convertToUnicode(const char* chars, int len, ConverterState *state) const
	{
		uchar buf[4];
		int nbuf = 0;
		...
		for (int i = 0; i < len; i++) {
			...
			case 1:
				...
				if (Is2ndByteIn2Bytes(ch)) {
					buf[1] = ch;
					int clen = 2;
					uint u = Gb18030ToUnicode(buf, clen);
					if (clen == 2) {
						resultData[unicodeLen] = ZValidChar(static_cast<ushort>(u));
						++unicodeLen;
					} else {
						resultData[unicodeLen] = replacement;
						++unicodeLen;
			...
			case 3:
				...
				if (Is4thByte(ch)) {
					...
					uint u = Gb18030ToUnicode(buf, clen);
					if (clen == 4) {
						resultData[unicodeLen] = ZValidChar(u);
						++unicodeLen;
					} else {
						resultData[unicodeLen] = replacement;
						++unicodeLen;
		...
		u16string result_str(result.data(),unicodeLen);
...
/*! \class GbkCodec
  ...
  \brief The GbkCodec class provides conversion to and from the Chinese
  GBK encoding.
  ...
  GB18030 is backward compatible to GBK.  For this reason the GbkCodec class
  is implemented in terms of the GB18030 codec and uses its 1-byte and
  2-byte portion for conversion from and to Unicode.

  The GbkCodec is kept mainly for compatibility reasons with older software.
*/
	...
	int GbkCodec::_mibEnum()
	{
		return 113;
	}

	string GbkCodec::_name()
	{
		return "GBK";
	}

	list<string> GbkCodec::_aliases()
	{
		list<string> list;
	...
	u16string GbkCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
	{
		uchar buf[2];
		int nbuf = 0;
		ushort replacement = SpecialCharacter::ReplacementCharacter;
	...
	string GbkCodec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
	{
		char replacement = '?';
		if (state) {
			if (state->flags & ConvertInvalidToNull)
...
/*! \class Gb2312Codec
  ...
  \brief The Gb2312Codec class provides conversion to and from the Chinese
  GB2312 encoding.
...
*/
	...
	int Gb2312Codec::_mibEnum()
	{
		return 2025;
	}

	string Gb2312Codec::_name()
	{
		return "GB2312";
	}


	u16string Gb2312Codec::convertToUnicode(const char* chars, int len, ConverterState *state) const
	{
		uchar buf[2];
		int nbuf = 0;
		ushort replacement = SpecialCharacter::ReplacementCharacter;
	...
	string Gb2312Codec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
	{
		char replacement = '?';
		if (state) {
			if (state->flags & ConvertInvalidToNull)
	...
	static uint16_t const ucs_to_gb18030[28839] = {
	...
	static uint16_t const gb18030_2byte_to_ucs[22046] = {
			/* GB 0x8140..0x817E */
	...
	static uint16_t const gb18030_4byte_to_ucs[6793] = {
			...
			if (Is2ndByteIn2Bytes(second)) {
				...
				if (Is3rdByte(third) && Is4thByte(fourth)) {
					...
					if (gb4lin <= 0x99FB) {
						...
						if ((uint8_t)(gb4lin & 0xFF) >= g2u.tblBegin &&
							(uint8_t)(gb4lin & 0xFF) <= g2u.tblEnd) {
						...
						len = 1;
						uni = SpecialCharacter::ReplacementCharacter;
					}
				}
				else {
					len = 1;
	...
	int UnicodeToGb18030(uint uni, uchar *gbchar) {
	...
	int UnicodeToGbk(uint uni, uchar *gbchar) {
