...
uint Ksc5601ToUnicode(uint code);

uint UnicodeToKsc5601(uint unicode);
...
string EucKrCodec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
...
u16string EucKrCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    uchar buf[2] = {0, 0};
    int nbuf = 0;
    ...
    for (int i=0; i<len; i++) {
        ...
        case 1:
            // KSC 5601
            if (IsEucChar(ch)) {
                uint u = Ksc5601ToUnicode((buf[0] << 8) |  ch);
                result += ZValidChar(u);
            } else {
                // Error
                result += replacement;
...
int EucKrCodec::_mibEnum()
{
  return 38;
}

string EucKrCodec::_name()
{
  return "EUC-KR";
}
...
{
#if 0
  printf("Ksc5601ToUnicode : code = %x, unicode = %x\n",
...
{
#if 0
  printf("UnicodeToKsc5601 : unicode = %x, %x\n",
...
int CP949Codec::_mibEnum()
{
  return -949; // CP949 has no MIBenum. So we have to use fake value.
}

string CP949Codec::_name()
{
    return "windows-949";
}

list<string> CP949Codec::_aliases()
{
    list<string> aliases;
    aliases.push_back("CP949");
    return aliases;
}
...
/*!
  \reimp
*/
string CP949Codec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
...
/*!
  \reimp
*/
u16string CP949Codec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    uchar buf[2] = {0, 0};
    int nbuf = 0;
    ushort replacement = SpecialCharacter::ReplacementCharacter;
        ...
        case 1:
            ...
            if (IsEucChar(ch) && !IsCP949Char(buf[0])) {
                ...
                if (internal_code < 0 || internal_code >= 8822) {
                    ...
                    result += ZValidChar(cp949_icode_to_unicode[internal_code]);
            }
            nbuf = 0;
            break;
        }
