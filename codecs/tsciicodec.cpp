...
static uchar UnicodeToTSCII(ushort u1, ushort u2, ushort u3);
static uint TSCIIToUnicode(uint code, uint *s);
...
/*!
    ...
    information.
*/
string TsciiCodec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (UCS2Tool::row(ch) == 0x00 && UCS2Tool::cell(ch) < 0x80) {
            ...
            i += 2;
                                          ...
                                          uc[i + 1], 0))) {
            i++;
        } else if ((j = UnicodeToTSCII(uc[i], 0, 0))) {
        } else {
            // Error
            j = replacement;
            ++invalid;
...
/*!
    ...
    to Unicode, and returns the result in a u16string. The \a state contains
    some conversion flags, and is used by the codec to maintain state
    information.
*/
u16string TsciiCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
...
static unsigned int TSCIIToUnicode(uint code, uint *s)
{
    int len = 0;
    for (int i = 0; i < 3; i++) {
