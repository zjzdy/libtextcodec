...
static int Big5hkscsToUnicode(const uchar *s, uint *pwc);
static int UnicodeToBig5hkscs(uint wc, uchar *r);
...
int Big5Codec::_mibEnum()
{
    return 2026;
}

string Big5Codec::_name()
{
    return "Big5";
}
...
static int Big5ToUnicode(const uchar *buf, uint *u)
{
    ...
    return Big5hkscsToUnicode(buf, u);
}

static int UnicodeToBig5(ushort ch, uchar *buf)
{
    ...
    return UnicodeToBig5hkscs(ch, buf);
}

u16string Big5Codec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
...
string Big5Codec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
    ...
    for (int i=0; i<len; i++) {
        ...
        if (ch < 0x80) {
            // ASCII
            *cursor++ = ch;
        } else if (UnicodeToBig5(ch, c) == 2 && c[0] >= 0xa1 && c[0] <= 0xf9) {
            *cursor++ = c[0];
            *cursor++ = c[1];
        } else {
            *cursor++ = replacement;
...
list<string> Big5Codec::_aliases()
{
    list<string> aliases;
    aliases.push_back("Big5-ETen");
    aliases.push_back("CP950");
    return aliases;
}

int Big5hkscsCodec::_mibEnum()
{
    return 2101;
}


string Big5hkscsCodec::_name()
{
    return "Big5-HKSCS";
}


u16string Big5hkscsCodec::convertToUnicode(const char* chars, int len, ConverterState *state) const
{
    uchar buf[2] = {0};
    int nbuf = 0;
    ushort replacement = SpecialCharacter::ReplacementCharacter;
...
string Big5hkscsCodec::convertFromUnicode(const ushort *uc, int len, ConverterState *state) const
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (ch < 0x80) {
            // ASCII
            *cursor++ = ch;
        } else if (UnicodeToBig5hkscs(ch, c) == 2) {
            // Big5-HKSCS
            *cursor++ = c[0];
            *cursor++ = c[1];
        } else {
...
int UnicodeToBig5hkscs (uint wc, uchar *r)
{
