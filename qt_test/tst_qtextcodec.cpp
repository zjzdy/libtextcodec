...
class Q_TextDecoder {
public:
    TextDecoder *m_tdecode;
    Q_TextDecoder(TextDecoder *td) {m_tdecode = td;}
    QString toUnicode(const QByteArray &ba) const {auto r = m_tdecode->toUnicode(ba.data());return QString((QChar *)r.data(),r.length());}
    QString toUnicode(const char* chars, int len) const {auto r = m_tdecode->toUnicode(chars,len);return QString((QChar *)r.data(),r.length());}
    void del() {delete m_tdecode;}
    void toUnicode(QString *target, const char *chars, int len) {*target += toUnicode(chars,len);}
};
class Q_TextCodec
{
public:
    TextCodec *m_tcodec;
    Q_TextCodec(TextCodec *c) {m_tcodec = c;}
    static Q_TextCodec codecForName(const QByteArray &name) {return Q_TextCodec(TextCodec::codecForName(name.toStdString()));}
    static Q_TextCodec codecForName(const char *name) {return Q_TextCodec(TextCodec::codecForName(name));}
    static Q_TextCodec codecForMib(int mib) {return Q_TextCodec(TextCodec::codecForMib(mib));}

    static QList<QByteArray> availableCodecs()
    {
        QList<std::basic_string<char>> list = QList<std::basic_string<char>>::fromStdList(TextCodec::availableCodecs());
        QList<QByteArray> result;
        for(auto i = 0;i < list.size();i++) {
            result.append(QByteArray::fromStdString(list.at(i)));
        }
        return result;
    }

    static QList<int> availableMibs() {return QList<int>::fromStdList(TextCodec::availableMibs());}

    static Q_TextCodec codecForLocale() {return Q_TextCodec(TextCodec::codecForLocale());}
    static void setCodecForLocale(Q_TextCodec c) {TextCodec::setCodecForLocale(c.m_tcodec);}
    static void setCodecForLocale(Q_TextCodec *c) {TextCodec::setCodecForLocale(c->m_tcodec);}

    static Q_TextCodec codecForHtml(const QByteArray &ba) {return Q_TextCodec(TextCodec::codecForHtml(ba.toStdString()));}
    static Q_TextCodec codecForHtml(const QByteArray &ba, Q_TextCodec defaultCodec) {return Q_TextCodec(TextCodec::codecForHtml(ba.toStdString(),defaultCodec.m_tcodec));}

    static Q_TextCodec codecForUtfText(const QByteArray &ba) {return Q_TextCodec(TextCodec::codecForUtfText(ba.toStdString()));}
    static Q_TextCodec codecForUtfText(const QByteArray &ba, Q_TextCodec defaultCodec) {return Q_TextCodec(TextCodec::codecForUtfText(ba.toStdString(),defaultCodec.m_tcodec));}

    bool canEncode(uint16_t t) const {return m_tcodec->canEncode(t);}
    bool canEncode(const QString &t) const {return m_tcodec->canEncode(std::basic_string<uint16_t>((uint16_t*)t.data(),t.length()));}

    QString toUnicode(const QByteArray &ba) const {auto r = m_tcodec->toUnicode(ba.data());return QString((QChar *)r.data(),r.length());}
    QString toUnicode(const char* chars) const {auto r = m_tcodec->toUnicode(chars);return QString((QChar *)r.data(),r.length());}
    QByteArray fromUnicode(const QString &t) const {return QByteArray::fromStdString(m_tcodec->fromUnicode(std::basic_string<uint16_t>((uint16_t*)t.data(),t.length())));}
    QString toUnicode(const char *in, int length, TextCodec::ConverterState *state = Q_NULLPTR) const
        {auto r = m_tcodec->toUnicode(in,length,state);return QString((QChar *)r.data(),r.length());}
    QByteArray fromUnicode(const QChar *in, int length, TextCodec::ConverterState *state = Q_NULLPTR) const
        {return QByteArray::fromStdString(m_tcodec->fromUnicode((uint16_t *)in,length,state));}

    Q_TextDecoder makeDecoder(TextCodec::ConversionFlags flags = TextCodec::DefaultConversion) const {return Q_TextDecoder(m_tcodec->makeDecoder(flags));}
    QByteArray name() const {return QByteArray::fromStdString(m_tcodec->name());}
    QList<QByteArray> aliases() const
    {
        QList<std::basic_string<char>> list = QList<std::basic_string<char>>::fromStdList(m_tcodec->aliases());
        QList<QByteArray> result;
        for(auto i = 0;i < list.size();i++) {
            result.append(QByteArray::fromStdString(list.at(i)));
        }
    ...
    int mibEnum() const {return m_tcodec->mibEnum();}
...
void tst_QTextCodec::toUnicode()
{
    ...
    if ( file.open( QIODevice::ReadOnly ) ) {
        ...
        QVERIFY(!ba.isEmpty());
        Q_TextCodec c = Q_TextCodec::codecForName( codecName.toLatin1() );
        QVERIFY(c.m_tcodec != 0);
...
void tst_QTextCodec::codecForName()
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForName(hint.toLatin1());
    if (actualCodecName.isEmpty()) {
        QVERIFY(!codec.m_tcodec);
    } else {
        QVERIFY(codec.m_tcodec != 0);
...
void tst_QTextCodec::fromUnicode()
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForName(codecName.toLatin1());
    if(codec.m_tcodec == 0)
        std::cout<<codecName.toStdString()<<" test fail"<<std::endl;
    QVERIFY(codec.m_tcodec != 0);
...
void tst_QTextCodec::toUnicode_codecForHtml()
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForHtml(data);
...
void tst_QTextCodec::toUnicode_incremental()
{
    ...
    QString incremental;
    Q_TextDecoder utf8Decoder = Q_TextCodec::codecForMib(106).makeDecoder();
...
{
    Q_TextCodec codec = Q_TextCodec::codecForLocale();
    QVERIFY(codec.m_tcodec != 0);
...
void tst_QTextCodec::asciiToIscii() const
{
    ...
    for(int i = 0; i < isciiCodecsLen; ++i) {
        ...
        const Q_TextCodec textCodec = Q_TextCodec::codecForName(isciiCodecs[i]);
        if (!textCodec.m_tcodec)
...
void tst_QTextCodec::nonFlaggedCodepointFFFF() const
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForMib(106); // UTF-8
    QVERIFY(codec.m_tcodec);
    ...
    TextCodec::ConverterState state(TextCodec::ConvertInvalidToNull);
...
{
    QVERIFY(Q_TextCodec::codecForName("UTF-16").aliases().isEmpty());
}

void tst_QTextCodec::mibForTSCII() const
{
    Q_TextCodec codec = Q_TextCodec::codecForName("TSCII");
    QVERIFY(codec.m_tcodec);
...
{
    Q_TextCodec codec = Q_TextCodec::codecForMib(2107);
    QVERIFY(codec.m_tcodec);
    QCOMPARE(codec.mibEnum(), 2107);
}
...
{
    Q_TextCodec codec = Q_TextCodec::codecForName("ISO8859-16");
    QVERIFY(codec.m_tcodec);
...
void tst_QTextCodec::utf8stateful()
{
    ...
    Q_TextCodec utf8codec = Q_TextCodec::codecForName("utf-8");
    QVERIFY(utf8codec.m_tcodec);
...
void tst_QTextCodec::utfHeaders()
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForName(codecName);
    QVERIFY(codec.m_tcodec != 0);

    QFETCH(int, flags);
    TextCodec::ConversionFlags cFlags = TextCodec::ConversionFlags(flags);
    TextCodec::ConverterState state(cFlags);
    ...
    if (toUnicode) {
        ...
        if (!rowName.endsWith("nobom") && !rowName.contains(ignoreReverseTestOn)) {
            TextCodec::ConverterState state2(cFlags);
...
void tst_QTextCodec::codecForHtml()
{
    ...
    if (defaultCodecMib != -1)
        QCOMPARE(Q_TextCodec::codecForHtml(html, Q_TextCodec::codecForMib(defaultCodecMib)).mibEnum(), expectedMibEnum);
    else // Test one parameter version when there is no default codec.
        QCOMPARE(Q_TextCodec::codecForHtml(html).mibEnum(), expectedMibEnum);
}
...
void tst_QTextCodec::codecForUtfText()
{
    ...
    Q_TextCodec codec = Q_TextCodec::codecForUtfText(encoded, 0);
    if (detected)
...
public:
    ...
    void run()
    {
        Q_TextCodec c = Q_TextCodec::codecForName(codecName);
        if (!c.m_tcodec) {
...
public:
    ...
    void run()
    {
        Q_TextCodec c = Q_TextCodec::codecForMib(mib);
        if (!c.m_tcodec) {
...
{
    QList<QByteArray> codecList = Q_TextCodec::availableCodecs();
    const QVector<int> mibList = Q_TextCodec::availableMibs().toVector();
...
{
    QVERIFY(!Q_TextCodec::codecForName("").m_tcodec);
    QVERIFY(!Q_TextCodec::codecForName(QByteArray()).m_tcodec);
    QVERIFY(!Q_TextCodec::codecForName("-").m_tcodec);
    QVERIFY(!Q_TextCodec::codecForName("\1a\2b\3a\4d\5c\6s\7a\xffr\xec_\x9c_").m_tcodec);
    QVERIFY(!Q_TextCodec::codecForName("\n").m_tcodec);
    QVERIFY(!Q_TextCodec::codecForName("don't exist").m_tcodec);
    QByteArray huge = "azertyuiop^$qsdfghjklm<wxcvbn,;:=1234567890�_";
    huge = huge + huge + huge + huge + huge + huge + huge + huge;
    huge = huge + huge + huge + huge + huge + huge + huge + huge;
    huge = huge + huge + huge + huge + huge + huge + huge + huge;
    huge = huge + huge + huge + huge + huge + huge + huge + huge;
    QVERIFY(!Q_TextCodec::codecForName(huge).m_tcodec);
}

void tst_QTextCodec::checkAliases_data()
{
    QTest::addColumn<QByteArray>("codecName");
    const QList<QByteArray> codecList = Q_TextCodec::availableCodecs();
...
void tst_QTextCodec::checkAliases()
{
    ...
    Q_TextCodec c = Q_TextCodec::codecForName(codecName);
    QVERIFY(c.m_tcodec);
    QCOMPARE(Q_TextCodec::codecForName(codecName).m_tcodec, c.m_tcodec);
    QCOMPARE(Q_TextCodec::codecForName(c.name()).m_tcodec, c.m_tcodec);

    const auto aliases = c.aliases();
    for (const QByteArray a : aliases) {
        QCOMPARE(Q_TextCodec::codecForName(a).m_tcodec, c.m_tcodec);
    }
}
...
void tst_QTextCodec::moreToFromUnicode()
{
    ...
    Q_TextCodec c = Q_TextCodec::codecForName( codecName.data() );
    QVERIFY(c.m_tcodec);
...
struct UserCodec : public TextCodec
{
    ...
    std::basic_string<char> name() const Q_DECL_OVERRIDE
    { return "UserCodec"; }
    std::list<std::basic_string<char>> aliases() const Q_DECL_OVERRIDE
    { return std::list<std::basic_string<char>>() = { "usercodec" , "user-codec"}; }
    int mibEnum() const Q_DECL_OVERRIDE
    { return 5000; }

    virtual std::basic_string<uint16_t> convertToUnicode(const char *, int, ConverterState *) const Q_DECL_OVERRIDE
    { return std::basic_string<uint16_t>(); }
    virtual std::basic_string<char> convertFromUnicode(const uint16_t *, int, ConverterState *) const Q_DECL_OVERRIDE
...
void tst_QTextCodec::userCodec()
{
    ...
    QVERIFY(!Q_TextCodec::availableCodecs().contains("UserCodec"));
    QVERIFY(!Q_TextCodec::codecForName("UserCodec").m_tcodec);

    Q_TextCodec codec = Q_TextCodec(new UserCodec);
    executedOnce = true;

    QList<QByteArray> availableCodecs = Q_TextCodec::availableCodecs();
    QVERIFY(availableCodecs.contains("UserCodec"));
    QVERIFY(availableCodecs.contains("usercodec"));
    QVERIFY(availableCodecs.contains("user-codec"));

    Q_TextCodec pcodec = Q_TextCodec::codecForName("UserCodec");
    QCOMPARE(pcodec.m_tcodec, codec.m_tcodec);

    pcodec = Q_TextCodec::codecForName("user-codec");
    QCOMPARE(pcodec.m_tcodec, codec.m_tcodec);

    pcodec = Q_TextCodec::codecForName("User-Codec");
    QCOMPARE(pcodec.m_tcodec, codec.m_tcodec);

    pcodec = Q_TextCodec::codecForMib(5000);
    QCOMPARE(pcodec.m_tcodec, codec.m_tcodec);
}

struct DontCrashAtExit {
    ~DontCrashAtExit() {
        Q_TextCodec c = Q_TextCodec::codecForName("utf8");
        if (c.m_tcodec)
            c.toUnicode("azerty");
